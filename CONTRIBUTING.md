...
Butler is a local-first AI butler: a 1920s English valet persona wrapped around
...
```bash
git clone --recurse-submodules git@github.com:miclaldogan/butler.git
cd butler
./setup.sh           # creates .venv, installs the package, sets up .env
source .venv/bin/activate
python -m butler --doctor     # sanity-check your environment
python -m butler              # run the terminal interface
```
...
- For the desktop UI: **Node + Rust/Tauri** (see `butler-ui/`).
...
└── __main__.py    # CLI entry (`python -m butler`)

butler-ui/          # Tauri v2 + React desktop "Operations Center" (talks over WS)
...
2. **CoT route** (`core/intent.py::cot_route`) — the LLM decides what to do and
   ...
   this is where you teach Butler to recognise new kinds of requests.
...
- **Intent routing** (`core/intent.py`) — Butler mis-routes a request to chat
  when it should call a tool (or vice-versa)? Add a routing hint/example and a
  test. High impact, low blast radius.
- **Tool reliability** (`src/butler/tools/`) — make a tool fail gracefully,
...
- **UI improvements** (`butler-ui/`) — the Operations Center has six pages
...
1. Create `src/butler/tools/your_tool.py`, subclass `BaseTool`, implement
...
In a real install, Butler runs as a **systemd user service** importing from an
**install clone** at `~/.local/share/butler/src` — *not* your dev checkout. If
...
(`systemctl --user {restart,status,stop} butler-daemon`) — don't hand-launch it,
