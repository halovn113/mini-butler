...
git add src/butler/tools/news.py .jules/bolt.md
git commit -m "⚡ Bolt: Implement concurrent Hacker News API fetches"

gh pr create --title "⚡ Bolt: Implement concurrent Hacker News API fetches" --body "💡 What: Modified \`_fetch_hn\` in \`src/butler/tools/news.py\` to fetch Hacker News story items concurrently using \`asyncio.gather\` with \`return_exceptions=True\`.
