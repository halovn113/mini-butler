...
logger = logging.getLogger("butler.live_ui")
...
class LiveUI:
    ...
    def _render_chat(self) -> Panel:
        ...
        for i, (role, msg) in enumerate(recent):
            if role == "user":
                ...
                if "```" in msg or len(msg) > 120:
                    parts.append(Text.from_markup("[bold cyan]◆ Butler[/]"))
                    parts.append(Markdown(msg))
                else:
                    parts.append(
                        Text.from_markup(
                            f"[bold cyan]◆ Butler[/]  {escape(msg)}"
                        )
                    )
            elif role == "system":
        ...
        if self._streaming_text is not None:
            ...
            parts.append(
                Text.from_markup(
                    f"[bold cyan]◆ Butler[/]  {escape(self._streaming_text)}▌"
                )
            )
        elif self._busy:
    ...
    def _start_input_thread(
        ...
        t = threading.Thread(target=_reader, daemon=True, name="butler-input")
        t.start()
        return t
        ...
        async def check_claude() -> None:
            try:
                if config.anthropic_api_key:
                    ...
                    self.add_log("✗ Claude: BUTLER_ANTHROPIC_API_KEY not set")
            except Exception:
                self._services[_llm_key] = ServiceDot.DOWN
        ...
        async def check_openai() -> None:
            try:
                if config.openai_api_key:
                    ...
                    self._services[_llm_key] = ServiceDot.UNCONFIGURED
                    self.add_log("✗ OpenAI: BUTLER_OPENAI_API_KEY not set")
            except Exception:
                self._services[_llm_key] = ServiceDot.DOWN
    ...
    async def _scheduler_loop(self) -> None:
        ...
        while self._running:
            ...
            try:
                from butler.core.digest import digest_manager
                for fn in (
    ...
    async def _chat_loop(self) -> None:
        ...
        self.add_chat("system", "Butler v3 started.")
        self.add_chat("system", f"Model: {_active_model_label()}")
        self.add_chat("system", "─" * 38)

        # ── First-run welcome banner ───────────────────────────────
        try:
            from butler.memory.onboarding import is_onboarding_done
            ...
            if not is_onboarding_done(_palace_parent):
                ...
                self.add_chat("system", "╔══ Welcome to Butler — First Run ══╗")
                ...
                self.add_chat("system",
                    ...
                    else "  ⚙   TTS (set BUTLER_TTS_ENABLED=true to enable)")
                self.add_chat("system", "  ⚙   Voice input — needs setup")
                self.add_chat("system", "╚══════════════════════════════════╝")
                self.add_chat("system",
                    "Run [bold]butler --setup onboarding[/] to personalise Butler "
        ...
        try:
            ...
            self.add_chat("butler", time_ctx.greeting_line())
        except Exception:
            pass
    ...
    async def _process_input(self, text: str) -> None:
        ...
        if result.stream is not None:
            ...
            if accumulated.strip():
                self.add_chat("butler", accumulated)
            else:
                self.add_chat(
                    "error",
        ...
        if result.needs_confirm:
            ...
            self.add_chat("butler", result.response)
