...
from butler.core.event_bus import bus, Event

log = logging.getLogger("butler.ws_server")
...
_CONFIG_KEY_MAP: dict[str, tuple[str, str]] = {
    "llm_provider":               ("BUTLER_LLM_PROVIDER",               "llm_provider"),
    "ollama_model":               ("BUTLER_OLLAMA_MODEL",               "ollama_model"),
    "anthropic_api_key":          ("BUTLER_ANTHROPIC_API_KEY",          "anthropic_api_key"),
    "anthropic_model":            ("BUTLER_ANTHROPIC_MODEL",            "anthropic_model"),
    "gemini_enabled":             ("BUTLER_GEMINI_ENABLED",             "gemini_enabled"),
    "gemini_api_key":             ("BUTLER_GEMINI_API_KEY",             "gemini_api_key"),
    "language":                   ("BUTLER_LANGUAGE",                   "language"),
    "tts_enabled":                ("BUTLER_TTS_ENABLED",                "tts_enabled"),
    "stt_enabled":                ("BUTLER_STT_ENABLED",                "stt_enabled"),
    "wake_word_enabled":          ("BUTLER_WAKE_WORD_ENABLED",          "wake_word_enabled"),
    "distillation_enabled":       ("BUTLER_DISTILLATION_ENABLED",       "distillation_enabled"),
    "shell_confirm_destructive":  ("BUTLER_SHELL_CONFIRM_DESTRUCTIVE",  "shell_confirm_destructive"),
    "observer_enabled":           ("BUTLER_OBSERVER_ENABLED",           "observer_enabled"),
    "verbosity":                  ("BUTLER_VERBOSITY",                  "verbosity"),
    "autonomy":                   ("BUTLER_AUTONOMY",                   "autonomy"),
    "mood_bias":                  ("BUTLER_MOOD_BIAS",                  "mood_bias"),
}
...
class WsBroadcastServer:
    ...
    async def _handle_chat(self, ws: ServerConnection, text: str) -> None:
        ...
        from butler.core.event_bus import bus as _event_bus, Event as _Event
    ...
    async def _preload_translation(self) -> None:
        ...
        try:
            from butler.i18n.bridge import bridge
...
def _compute_anomalies(cpu: float, ram_pct: float) -> list[dict]:
    ...
    for entry in _recent_logs:
        ...
        source = msg.split(":", 1)[0].strip() if ":" in msg else "butler"
