...
def _handle_setup(parts: list[str]) -> None:
    ...
    if len(parts) >= 2 and parts[0].lower() == "google":
        ...
        from butler.auth.google_oauth import setup_google
...
def _setup_telegram() -> None:
    ...
    print("Start with: python -m butler.interface.telegram_bot")
...
def _setup_claude() -> None:
    ...
    _write_env({
        "BUTLER_LLM_PROVIDER":    "claude",
        "BUTLER_ANTHROPIC_API_KEY": api_key,
        "BUTLER_ANTHROPIC_MODEL": model,
    })
...
def _setup_openai() -> None:
    ...
    updates = {
        "BUTLER_LLM_PROVIDER":  "openai",
        "BUTLER_OPENAI_API_KEY": api_key,
        "BUTLER_OPENAI_MODEL":   model,
    }
    if base_url:
        updates["BUTLER_OPENAI_BASE_URL"] = base_url
...
def _setup_gemini() -> None:
    ...
    _write_env({
        "BUTLER_LLM_PROVIDER":  "gemini",
        "BUTLER_GEMINI_ENABLED": "true",
        "BUTLER_GEMINI_API_KEY": api_key,
        "BUTLER_GEMINI_MODEL":   model,
    })
...
_VOICE_ENV_PREFIXES: tuple[str, ...] = (
    "BUTLER_VOICE_ENABLED=",
    "BUTLER_TTS_ENABLED=",
    "BUTLER_STT_ENABLED=",
    "BUTLER_GHOST_LOOP_ENABLED=",
    "BUTLER_WAKE_WORD_ENABLED=",
    "BUTLER_PICOVOICE_ACCESS_KEY=",
)
...
def _setup_voice() -> None:
    ...
    3. Prompts for ``BUTLER_PICOVOICE_ACCESS_KEY`` (wake-word detection).
    4. Writes ``BUTLER_VOICE_ENABLED``, ``BUTLER_TTS_ENABLED``,
       ``BUTLER_STT_ENABLED``, ``BUTLER_GHOST_LOOP_ENABLED`` (and
       ``BUTLER_WAKE_WORD_ENABLED`` when a Picovoice key is supplied) to
       ``.env``.
    ...
    lines.append("BUTLER_VOICE_ENABLED=true")
    lines.append("BUTLER_TTS_ENABLED=true")
    lines.append("BUTLER_STT_ENABLED=true")
    lines.append("BUTLER_GHOST_LOOP_ENABLED=true")
    if picovoice_key:
        lines.append(f"BUTLER_PICOVOICE_ACCESS_KEY={picovoice_key}")
        lines.append("BUTLER_WAKE_WORD_ENABLED=true")
...
def _write_location_to_env(city: str, lat: float, lon: float) -> None:
    """Write BUTLER_CITY / BUTLER_LAT / BUTLER_LON to .env."""
    from pathlib import Path
    ...
    lines = [
        line for line in lines
        if not line.startswith("BUTLER_CITY=")
        and not line.startswith("BUTLER_LAT=")
        and not line.startswith("BUTLER_LON=")
    ]

    lines.append(f"BUTLER_CITY={city}")
    lines.append(f"BUTLER_LAT={lat}")
    lines.append(f"BUTLER_LON={lon}")
...
def _setup_profile() -> None:
    ...
    from butler.core.profile import profile, ALL_BRIEFING_SECTIONS, ALL_NEWS_SOURCES
...
def _setup_schedule() -> None:
    ...
    import json
    import os
    from butler.core.schedule import Schedule, DAYS_EN, DAYS_TR
...
def _show_config() -> None:
    ...
    from butler.config import Config, config

    print("Butler v2 — Current Configuration")
    print("─" * 52)
...
async def _doctor() -> None:
    ...
    from butler.auth.token_store import token_store

    print("Butler v2 — System Check")
    print("─" * 52)
    ...
    if _provider == "claude":
        from butler.llm.anthropic_client import claude as _claude
        ...
        if _ok:
            ...
            print("❌ Gemini: UNREACHABLE — check BUTLER_GEMINI_API_KEY")
    else:
        # Ollama (default)
    ...
    if config.mempalace_enabled:
        ...
        print("⚪ MemPalace: disabled  → BUTLER_MEMPALACE_ENABLED=true")

    # Vision / VLM
    if config.vlm_enabled:
        print(f"✅ Vision/VLM: enabled ({config.vlm_endpoint})")
    else:
        print("⚪ Vision/VLM: disabled  → BUTLER_VLM_ENABLED=true")
    ...
    # Profile
    from butler.core.profile import profile as _prof
    ...
    try:
        from butler.tools.accessibility import _init_atspi, list_applications, detect_display_server
    ...
    if config.input_control_enabled:
        ...
        print("⚪ Input Control: disabled  → BUTLER_INPUT_CONTROL_ENABLED=true")

    # Spatial Cache (#121)
    try:
        from butler.vision.spatial_cache import spatial_db as _sc
    ...
    try:
        from butler.vision.navigator import navigator as _nav
    ...
    if config.observer_enabled:
        ...
        print("⚪ Observer: disabled  → BUTLER_OBSERVER_ENABLED=true")

    # Affinity Engine (#221)
    if config.rl_enabled:
        try:
            from butler.agent.affinity_engine import affinity_engine as _ae
            _ae.init(config.db_path)
            print(f"✅ Affinity Engine: {_ae.status_line()}")
        except Exception:
            print("❌ Affinity Engine: enabled but init failed")
    else:
        print("⚪ Affinity Engine: disabled  → BUTLER_RL_ENABLED=true")

    # Intervention Queue (#126)
    if config.rl_enabled:
        try:
            from butler.agent.interventions import intervention_queue as _ivq
    ...
    if config.app_detector_enabled:
        try:
            from butler.agent.app_detector import app_detector as _ad
            _ad.init(
        ...
        print("⚪ App Detector: disabled  → BUTLER_APP_DETECTOR_ENABLED=true")

    # Desktop Notifications (#153)
    if config.desktop_notifications:
        try:
            from butler.agent.notifier import notifier as _dn
            _dn.init(
        ...
        print("⚪ Notifications: disabled  → BUTLER_DESKTOP_NOTIFICATIONS=true")

    # ── Voice Pipeline (#277 — consolidated diagnostics) ───────────────
    print()
    _voice_on = config.voice_enabled
    if _voice_on:
        print("🎙️  Voice Master Switch: ON  (BUTLER_VOICE_ENABLED=true)")
    else:
        _any_voice = any([
        ...
        if _any_voice:
            ...
            print("⚪ Voice: disabled  → BUTLER_VOICE_ENABLED=true")
    ...
    if config.tts_enabled:
        try:
            from butler.agent.tts import tts_engine as _tts
            if _tts.available():
        ...
        print("  ⚪ TTS: disabled  → BUTLER_TTS_ENABLED=true")

    # Audio Ducking (#171)
    if config.audio_duck_enabled:
        try:
            from butler.agent.audio_ducker import audio_ducker as _ducker
        ...
        print("  ⚪ Audio Ducking: disabled  → BUTLER_AUDIO_DUCK_ENABLED=true")

    # Wake Word Detection (#165)
    if config.wake_word_enabled:
        if not config.picovoice_access_key:
            print("  ❌ Wake Word: enabled but BUTLER_PICOVOICE_ACCESS_KEY not set")
        ...
        print("  ⚪ Wake Word: disabled  → BUTLER_WAKE_WORD_ENABLED=true")

    # Ghost Loop / STT (#36) — Whisper model check
    if config.ghost_loop_enabled and config.stt_enabled:
        try:
            from butler.agent.ghost_loop import ghost_loop as _gl
            diag = _gl.diagnose()
        ...
        print("  ❌ Ghost Loop: enabled but BUTLER_STT_ENABLED=false")
    else:
        print("  ⚪ Ghost Loop: disabled  → BUTLER_GHOST_LOOP_ENABLED=true + BUTLER_STT_ENABLED=true")

    # Ambient Sound Analysis (#166, #441)
    if config.ambient_enabled:
        if not config.wake_word_enabled:
            try:
                from butler.agent.ambient import ambient_analyzer as _amb
                diag = _amb.diagnose()
        ...
        print("  ⚪ Ambient: disabled  → BUTLER_AMBIENT_ENABLED=true")
    ...
    # Places
    from butler.core.places import places as _plc
    ...
    # Scheduler
    from butler.core.scheduler import scheduler as _sched
    ...
    if config.job_scheduler_enabled:
        try:
            from butler.agent.job_scheduler import job_scheduler as _js
        ...
        print("⚪ Job Scheduler: disabled  → BUTLER_JOB_SCHEDULER_ENABLED=true")
...
def _setup_systemd() -> None:
    ...
    # src/butler/cli/setup.py → repo root is 4 levels up
...
"""
    ...
    if answer in ("", "y", "yes"):
        ...
        print("  systemctl --user enable butler.service")
        print("  systemctl --user start butler.service")
...
def _systemd_check() -> None:
    ...
    # 1. Service file
    service_path = Path.home() / ".config" / "systemd" / "user" / "butler.service"
    ...
    journal = subprocess.run(
        ["journalctl", "--user", "-u", "butler.service",
    ...
    enabled_result = subprocess.run(
        ["systemctl", "--user", "is-enabled", "butler.service"],
        capture_output=True, text=True,
    )
    ...
    if en_state == "enabled":
        ...
        print(f"⚪ Boot: {en_state} — run: systemctl --user enable butler.service")
