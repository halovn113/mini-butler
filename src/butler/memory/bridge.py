...
log = logging.getLogger("butler.memory.bridge")
...
class MemPalaceBridge:
    ...
    async def init(self) -> None:
        ...
        try:
            ...
            self._wing = getattr(cfg, "mempalace_wing", "butler")
            ...
            if not is_onboarding_done(palace_parent):
                try:
                    import sys
                    if sys.stdin.isatty():
                        from butler.memory.onboarding import run_onboarding
                        run_onboarding(
                            identity_path=identity_path,
                            kg=self._kg,
    ...
    def _store_exchange_sync(
        ...
        add_drawer(
            ...
            room=room,
            content=exchange,
            source_file=f"butler_session_{ts[:10]}",
    ...
    def _distill_session_sync(self, session_id: int) -> dict:
        ...
        for i, exchange in enumerate(exchanges):
            ...
            success = add_drawer(
                ...
                room=room,
                content=text,
                source_file=f"butler_session_{session_id}",
                chunk_index=i,
                agent="butler_distiller",
            )
            if success:
                drawers_added += 1
