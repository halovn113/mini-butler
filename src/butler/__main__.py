"""
Butler v2 — Entry point

Commands:
  butler                         → TUI
  butler --ui                    → desktop UI (Tauri; starts daemon if not running)
  butler --once "query"          → single query, no UI
  butler --daemon                → headless daemon (scheduler + GPS, no TUI)
  butler --doctor                → system health check
  butler --setup onboarding         → first-run personalization wizard
  butler --setup profile         → user profile setup
  butler --setup google gmail    → OAuth setup for Gmail
  butler --setup google classroom → OAuth setup for Classroom
  butler --setup schedule        → class schedule setup
  butler --setup telegram        → Telegram bot token setup
  butler --setup places          → Known locations setup
  butler --setup gemini          → Gemini API key setup
  butler --setup voice           → Guided voice setup wizard (STT + TTS + wake word)
  butler --setup systemd         → install systemd user service
"""
...
from butler.cli.setup import _handle_setup, _doctor, _show_config, _cache_stats


def main() -> None:
    parser = argparse.ArgumentParser(prog="butler", description="Butler v2 — your terminal host")
    ...
    parser.add_argument("--ui", action="store_true",
                        help="Launch the Butler desktop UI (Tauri)")
    ...
    from butler.interface.live_ui import run
    run()
...
def _open_ui() -> None:
    ...
    # butler-ui/ sits next to the repo root (3 levels up from src/butler/__main__.py)
    ui_dir = Path(__file__).resolve().parent.parent.parent / "butler-ui"
    if not ui_dir.exists():
        sys.exit(
            f"[✗] butler-ui directory not found at {ui_dir}\n"
            "    Manual fallback: cd ~/butler/butler-ui && npm run tauri:dev"
        )
    ...
    if not _ws_up():
        print("[→] Backend not running — starting butler --daemon …")
        subprocess.Popen(
            [sys.executable, "-m", "butler", "--daemon"],
    ...
    bin_name = "butler-ui.exe" if is_win else "butler-ui"
    ...
    print("[→] Starting Butler UI (dev mode) …")
...
async def _start_ws_server() -> None:
    ...
    try:
        from butler.interface.ws_server import ws_server
        await ws_server.start()
    except Exception as exc:
        import logging
        logging.getLogger("butler.main").warning("WS server failed to start: %s", exc)


async def _list_jobs() -> None:
    """List all scheduled APScheduler jobs (butler --jobs)."""
...
async def _run_job(job_id: str) -> None:
    """Manually trigger a job (butler --run-job <id>)."""
    from butler.config import config
    config.ensure_dirs()
...
async def _maintenance(dry_run: bool) -> None:
    """Run the 6-step nightly maintenance workflow (butler --maintenance)."""
    from butler.config import config
    config.ensure_dirs()
...
async def _reflect(dry_run: bool) -> None:
    """Run the nightly memory reflection workflow (butler --reflect)."""
    from butler.config import config
    config.ensure_dirs()
...
def _view_reflections() -> None:
    """View past daily reflections (butler --reflections)."""
    from butler.config import config
    config.ensure_dirs()
    ...
    if not reflections:
        print("No reflections yet. Run 'butler --reflect' first.")
        return
...
async def _overnight_poll(dry_run: bool) -> None:
    """Run one overnight poll cycle (butler --overnight-poll)."""
    from butler.config import config
    config.ensure_dirs()
...
def _mood_history() -> None:
    """Show 24h mood transition history (butler --mood-history)."""
...
async def _once(query: str) -> None:
    ...
    if result.needs_confirm and result.pending_tool and result.pending_args:
        ...
        if answer in ("evet", "e", "yes", "y", "ok", "tamam"):
            from butler.tools import registry as _reg
...
async def _daemon() -> None:
    """Run Butler as a headless daemon — APScheduler-driven.
    ...
    log = logging.getLogger("butler.daemon")
    log.info("Butler daemon starting (APScheduler)...")
    ...
    if not config.job_scheduler_enabled:
        ...
        async def _briefing_loop():
            while not stop_event.is_set():
                try:
                    from butler.personality.greeting import greeting_manager
    ...
    if gps_ok:
        try:
            ...
            pass
    log.info("Butler daemon stopped.")
