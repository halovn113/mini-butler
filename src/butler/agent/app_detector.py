...
def _parse_hyprctl(output: str) -> Optional[WindowInfo]:
    try:
        ...
        return None
    return WindowInfo(
        name=d.get("class", "") or d.get("initialClass", ""),
        title=d.get("title", ""),
        pid=d.get("pid", 0),
...
def _parse_hyprctl_clients(output: str) -> list[str]:
    ...
    return sorted(
        {
            c.get("class", "") or c.get("initialClass", "")
            for c in clients
            if c.get("class") or c.get("initialClass")
        }
    )
...
def _atspi_active_window() -> Optional[WindowInfo]:
    try:
        from butler.tools.accessibility import _init_atspi, _atspi, _get_desktop
...
def _atspi_running_apps() -> list[str]:
    try:
        from butler.tools.accessibility import list_applications
...
def parse_browser_context(title: str) -> dict[str, str]:
    ...
    for pat, act, ctx in _BROWSER_TITLE_PATTERNS:
        if pat.search(title):
            r["activity"], r["context"] = act.value, ctx
            break
...
def parse_ide_context(win: WindowInfo) -> dict[str, str]:
    ...
    if any(x in nl for x in ("code", "vscodium")):
        ...
        if m:
            r["file"], r["project"] = m.group(1), m.group(2)
        if r["project"]:
...
def _start_x11_listener(on_change: Callable[[], None], stop: threading.Event) -> bool:
    ...
    threading.Thread(target=_listen, name="butler-x11-listener", daemon=True).start()
    return True
...
def _start_dbus_listener(on_change: Callable[[], None], stop: threading.Event) -> bool:
        ...
        async def _run() -> None:
            try:
                ...
                try:
                    ...
                    proxy.get_interface("org.gnome.Shell").on_focus_app_changed(
                        lambda _: on_change()
                    )
                ...
                try:
                    ...
                    proxy.get_interface("org.kde.KWin").on_active_window_changed(
                        lambda: on_change()
                    )
    ...
    threading.Thread(target=_listen, name="butler-dbus-listener", daemon=True).start()
    return True


def _start_slow_poll(
    on_change: Callable[[], None], stop: threading.Event, interval: float = 5.0
) -> bool:
    ...
    threading.Thread(target=_poll, name="butler-slow-poll", daemon=True).start()
    return True
...
class AppDetector:
    ...
    def get_workspace_context(self) -> dict[str, Any]:
        ...
        if not self._docker_cache.is_valid(30.0):
            self._docker_cache = _CachedResult(
                value=_get_docker_containers(), timestamp=time.time()
            )
    ...
    def status_line(self) -> str:
        ...
        return (
            f"display={self._display_server} active={win.name if win else 'none'} "
