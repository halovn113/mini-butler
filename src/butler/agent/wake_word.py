...
log = logging.getLogger("butler.wake_word")
...
class WakeWordListener:
    ...
    def _find_keyword_path(self) -> Optional[str]:
        ...
        """
        _PATTERNS = (
            "hey-butler*.ppn", "hey_butler*.ppn", "Hey-Butler*.ppn",
            "Butler*.ppn", "butler*.ppn",
        )
        try:
            from butler.config import config
            base_dir = (
    ...
    def _init_porcupine(self) -> bool:
        ...
        if not access_key:
            log.warning("Wake word: BUTLER_PICOVOICE_ACCESS_KEY not set")
            return False
    ...
    def _init_audio(self) -> bool:
        ...
        try:
            from butler.agent.voice_capture import suppress_alsa_stderr
