...
log = logging.getLogger("butler.job_scheduler")
...
@contextmanager
def inhibit_sleep(reason: str = "Butler night task"):
...
async def _job_reminder_check() -> None:
    ...
    try:
        from butler.core.scheduler import scheduler as _old_scheduler
...
async def _job_briefing_watcher() -> None:
    ...
    try:
        from butler.agent.app_detector import app_detector, Activity
...
async def _job_proactive_engagement() -> None:
    ...
    """
    try:
        from butler.agent.proactive import proactive_engine
