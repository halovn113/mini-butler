...
Architecture:
    ...
    │   ├─ skip entries with application.name = "ButlerTTS"     │
...
log = logging.getLogger("butler.audio_ducker")

# ── Butler TTS label — must match PULSE_PROP in tts.py ──────────────────
_BUTLER_APP_NAME = "ButlerTTS"
...
class AudioDucker:
    ...
    def duck(self) -> bool:
        ...
        with self._lock:
            ...
            targets = [
                si for si in inputs
                if si["app_name"] != _BUTLER_APP_NAME
            ]
            if not targets:
                return False
    ...
    def _load_config(self) -> None:
        """Load ducking config from butler config."""
        try:
