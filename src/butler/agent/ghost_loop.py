...
log = logging.getLogger("butler.ghost_loop")
    ...
    def start(self) -> bool:
        ...
        try:
            ...
            if not config.ghost_loop_enabled:
                log.debug("Ghost Loop: disabled (BUTLER_GHOST_LOOP_ENABLED=false)")
                return False
            if not config.stt_enabled:
                log.debug("Ghost Loop: STT disabled (BUTLER_STT_ENABLED=false)")
                return False
        except Exception:
            return False
