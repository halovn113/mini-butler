...
log = logging.getLogger("butler.accessibility")
...
class AccessibilityTool(BaseTool):
        ...
        if not _init_atspi():
            ...
            return ToolResult(
                ...
                error=(
                    ...
                    "Alternatively, set BUTLER_VLM_ENABLED=true for screenshot-based fallback."
                ),
            )
    ...
    async def _vlm_fallback(
        ...
        """
        try:
            from butler.vision.remote_vlm import (
                analyze_screenshot, spatial_cache,
            )
            from butler.vision.screenshot import capture_window_base64, capture_base64
        except ImportError:
            return ToolResult(
                success=False, output="",
    ...
    async def _screenshot_analyze(
        ...
        if not self._vlm_available():
            return ToolResult(
                success=False, output="",
                error="VLM is disabled. Set BUTLER_VLM_ENABLED=true.",
            )
