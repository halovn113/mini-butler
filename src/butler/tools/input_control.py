...
Usage:
    from butler.tools.input_control import InputControlTool
"""
...
log = logging.getLogger("butler.input_control")
...
class InputControlTool(BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        if not config.input_control_enabled:
            return ToolResult(
                success=False, output="",
                error="Input control is disabled. Set BUTLER_INPUT_CONTROL_ENABLED=true.",
            )
