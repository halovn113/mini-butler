...
from butler.agent.env_probe import env as _env_snapshot
from butler.config import config
from butler.tools import BaseTool, ToolResult, registry

log = logging.getLogger("butler.tools.browser_control")
...
class BrowserControlTool(BaseTool):
    ...
    async def execute(self, action: str = "", **kwargs: Any) -> ToolResult:
        ...
        if not config.input_control_enabled and action not in ("screenshot", "open"):
            return ToolResult(
                ...
                error="Input control disabled. Set BUTLER_INPUT_CONTROL_ENABLED=true.",
            )
    ...
    async def _screenshot(self, kwargs: dict) -> ToolResult:
        ...
        try:
            if app:
                from butler.vision.screenshot import capture_window_base64
    ...
    async def _navigate(self, kwargs: dict) -> ToolResult:
        ...
        from butler.tools.input_control import hotkey
    ...
    async def _wait_for_load(self, kwargs: dict) -> ToolResult:
        ...
        from butler.vision.browser_vision import wait_for_load
    ...
    async def _find_and_click(self, kwargs: dict) -> ToolResult:
        ...
        for hint, keys in _HOTKEYS.items():
            if hint in target_lower or target_lower in hint:
                from butler.tools.input_control import hotkey as do_hotkey
        ...
        try:
            ...
            if nav.found:
                from butler.tools.input_control import click, double_click, right_click
                cx, cy = nav.center
        ...
        try:
            from butler.vision.browser_vision import find_and_click_element
    ...
    async def _type_in_element(self, kwargs: dict) -> ToolResult:
        ...
        if is_search_target:
            ...
            if search_url:
                ...
                _nav = await self._navigate({"url": search_url, "app": app, "wait": 2.5})
    ...
    async def _scroll(self, kwargs: dict) -> ToolResult:
        ...
        from butler.tools.input_control import scroll
