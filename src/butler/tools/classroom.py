"""
Butler v2 — Google Classroom Tool
Courses, assignments, announcements.
Uses classroom_token.json (separate school account).

Triggers: assignment, announcement, classroom, due date, courses, my courses
"""
...
from butler.auth.token_store import token_store, TokenNotFoundError
from butler.tools import BaseTool, ToolResult, registry

CLASSROOM_SUMMARY_PROMPT = """\
You are Bantz. Summarize these assignments concisely.
...
class ClassroomTool(BaseTool):
    ...
    description = (
        ...
        "Use for: assignments, homework, announcements, classroom, due dates, "
        "my courses, course list, anything due today."
    )
    risk_level = "safe"
    ...
    async def _get_courses(self, creds) -> ToolResult:
        ...
        return ToolResult(
            success=True,
            output=f"You're enrolled in {len(courses)} courses:\n" + "\n".join(lines),
    ...
    async def _get_assignments(self, creds) -> ToolResult:
        ...
        for a in assignments:
            ...
            if due:
                ...
                upcoming.append(f"  ⬜ No date: {title}  [{course}]")
