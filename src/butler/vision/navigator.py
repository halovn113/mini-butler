...
log = logging.getLogger("butler.vision.navigator")
...
class Navigator:
    ...
    def _try_atspi(
        ...
        try:
            from butler.tools.accessibility import find_element
    ...
    async def _do_click(self, x: int, y: int) -> None:
        ...
        from butler.tools.input_control import click
        await click(x, y)

    async def _do_double_click(self, x: int, y: int) -> None:
        from butler.tools.input_control import double_click
        await double_click(x, y)

    async def _do_right_click(self, x: int, y: int) -> None:
        from butler.tools.input_control import right_click
        await right_click(x, y)

    async def _do_type(self, text: str) -> None:
        from butler.tools.input_control import type_text
        from butler.config import config
