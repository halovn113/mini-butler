...
Usage:
    from butler.vision.remote_vlm import analyze_screenshot, VLMResult
...
log = logging.getLogger("butler.vision.vlm")
...
async def analyze_screenshot(
    ...
    Backend selection (BUTLER_VLM_BACKEND):
      "ollama" (default) — uses local Ollama with BUTLER_VLM_MODEL (default: llava).
                           VLM is auto-enabled; no remote server required.
      "remote"           — POSTs to BUTLER_VLM_ENDPOINT, falls back to Ollama on failure.
    ...
    if not config.vlm_enabled:
        return VLMResult(
            success=False,
            error="VLM is disabled. Set BUTLER_VLM_ENABLED=true (or BUTLER_VLM_BACKEND=ollama) to enable.",
        )
