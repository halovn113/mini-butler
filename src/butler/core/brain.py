...
from butler.core.time_context import time_ctx
from butler.data import data_layer
from butler.core.profile import profile
from butler.core.intent import cot_route, _ANAPHORA
from butler.core.finalizer import (
...
from butler.core.translation_layer import (  # noqa: F401
...
log = logging.getLogger("butler.brain")
...
class Brain:
    def __init__(self) -> None:
        import butler.tools.shell        # noqa: F401
        import butler.tools.system       # noqa: F401
        import butler.tools.filesystem   # noqa: F401
        import butler.tools.weather      # noqa: F401
        try:
            import butler.tools.news         # noqa: F401
        except (ImportError, ModuleNotFoundError):
            pass  # defusedxml may not be installed
        import butler.tools.web_search   # noqa: F401
        import butler.tools.web_reader   # noqa: F401
        import butler.tools.gmail        # noqa: F401
        import butler.tools.calendar     # noqa: F401
        import butler.tools.classroom    # noqa: F401
        import butler.tools.reminder     # noqa: F401
        try:
            import butler.tools.document     # noqa: F401
        except (ImportError, ModuleNotFoundError):
            pass  # PDF/DOCX deps may not be installed
        try:
            import butler.tools.accessibility  # noqa: F401
        except (ImportError, ModuleNotFoundError):
            pass  # AT-SPI2/gi deps may not be installed
        # gui_action removed (#185) — superseded by visual_click
        try:
            import butler.tools.visual_click  # noqa: F401  (#185)
        except (ImportError, ModuleNotFoundError):
            pass
        try:
            ...
            pass
        try:
            import butler.tools.screenshot_tool  # noqa: F401
        except (ImportError, ModuleNotFoundError):
            pass
        try:
            import butler.tools.desktop  # noqa: F401  (#322)
        except (ImportError, ModuleNotFoundError):
            pass
        try:
            import butler.tools.delegate_task  # noqa: F401  (#321)
        except (ImportError, ModuleNotFoundError):
            pass
        try:
            import butler.tools.workflow_tool  # noqa: F401  (#323)
        except (ImportError, ModuleNotFoundError):
            pass
        import butler.tools.summarizer    # noqa: F401  (Architect's Revision)
        try:
            import butler.tools.screen_query_tool  # noqa: F401  (vision pipeline)
            import butler.tools.vision_execute     # noqa: F401  (vision pipeline)
        except (ImportError, ModuleNotFoundError):
    ...
    def _ensure_memory(self) -> None:
        if not self._memory_ready:
            ...
            if config.multi_agent_enabled:
                try:
                    from butler.agent.agent_manager import agent_manager
                    agent_manager.init()
    ...
    def _start_awareness(self) -> None:
        ...
        if self._awareness_task is not None:
            return
        try:
            from butler.agent.awareness import awareness_collector
    ...
    def _desktop_context(self) -> str:
        ...
        from butler.core.memory_injector import desktop_context
    ...
    async def _vector_context(self, user_msg: str, limit: int = 3) -> str:
        ...
        from butler.core.memory_injector import vector_context
    ...
    def _get_bridge(self):
        ...
        from butler.core.translation_layer import get_bridge
        return get_bridge()

    async def _to_en(self, text: str) -> str:
        """Delegate to translation_layer (#226)."""
        from butler.core.translation_layer import to_en
        return await to_en(text)

    def _resolve_message_ref(self, text: str) -> str | None:
        """Delegate to translation_layer (#226)."""
        from butler.core.translation_layer import resolve_message_ref
    ...
    async def _vlm_describe_screen(self, img_bytes: bytes) -> None:
        ...
        try:
            ...
            from butler.config import config as _cfg
            if not _cfg.vlm_enabled:
                return
            from butler.vision.remote_vlm import describe_screen
    ...
    def _rl_reward_hook(self, tool_name: str, result: ToolResult) -> None:
        ...
        try:
            from butler.core.rl_hooks import rl_reward_hook
    ...
    def _push_toast(
        ...
        from butler.core.notification_manager import push_toast
    ...
    async def process(
        ...
        if feedback:
            ...
            try:
                from butler.core.rl_hooks import rl_feedback_reward
        ...
        if tool_name:
            ...
            if matched and hasattr(matched, "name") and isinstance(matched.name, str):
                ...
                from butler.tools import _normalise_tool_name
        ...
        if pre_line:
            try:
                from butler.core.event_bus import bus as _bus
        ...
        if result.success and result.data and result.data.get("screenshot"):
            ...
            if _cfg.screenshot_auto_after_action and any(
                ...
                try:
                    ...
                    from butler.vision import screenshot as _ss
    ...
    async def _chat(self, en_input: str, tc: dict) -> str:
        ...
        from butler.llm.router import get_provider
        try:
    ...
    async def _finalize(self, en_input: str, result: ToolResult, tc: dict) -> str:
        ...
        from butler.memory.omni_memory import omni_memory
