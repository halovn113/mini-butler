...
Backward compatibility:
  ...
  until they migrate to ``from butler.core.notification_manager import …``.
...
log = logging.getLogger("butler.core.notifications")
...
def notify_toast(title: str, reason: str = "", toast_type: str = "info") -> None:
    ...
    try:
        ...
        if notifier.enabled:
            notifier.send(f"Butler: {title}", reason or "")
    except Exception:
        pass
