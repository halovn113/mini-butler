...
Config (env / .env):
    BUTLER_DAILY_DIGEST_ENABLED=true
    BUTLER_DAILY_DIGEST_HOUR=20
    BUTLER_DAILY_DIGEST_MINUTE=0
    BUTLER_WEEKLY_DIGEST_ENABLED=true
    BUTLER_WEEKLY_DIGEST_DAY=sunday
    BUTLER_WEEKLY_DIGEST_HOUR=20
    BUTLER_WEEKLY_DIGEST_MINUTE=0
"""
...
log = logging.getLogger("butler.digest")
...
class DigestManager:
    ...
    async def _get_email_stats_today(self) -> Optional[str]:
        try:
            from butler.tools.gmail import email_stats_today
    ...
    async def _get_email_stats_week(self) -> Optional[str]:
        try:
            from butler.tools.gmail import email_stats_week
    ...
    async def _get_tomorrow_weather(self) -> Optional[str]:
        try:
            from butler.tools.weather import tomorrow_forecast
