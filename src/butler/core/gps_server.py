"""
Butler v2 — Phone GPS Receiver (LAN + Relay)
...
def _ensure_relay_token() -> str:
    ...
    if TOKEN_FILE.exists():
        ...
        if token:
            return token
    token = "butler-gps-" + secrets.token_urlsafe(8)
...
<div class="card">
  <div class="icon">&#x1F4CD;</div>
  <h1>Butler GPS</h1>
  <p class="subtitle">Share your location with Butler</p>
...
<title>Butler GPS Tracker</title>
<style>
*{margin:0;padding:0;box-sizing:border-box}
...
// ── Command listener (auto-start/stop from Butler) ──
...
class _GPSHandler(BaseHTTPRequestHandler):
    ...
    def _serve_phone_app(self):
        ...
        self.send_header(
            "Content-Disposition",
            'attachment; filename="butler-gps.html"',
        )
        self.end_headers()
        self.wfile.write(html.encode())
    ...
    def _serve_manifest(self):
        ...
        manifest = json.dumps({
            "name": "Butler GPS Tracker",
            "short_name": "Butler GPS",
            "description": "Adaptive GPS tracker for Butler",
            "start_url": "/pwa",
            "display": "standalone",
    ...
    def _serve_sw(self):
        ...
        sw_js = (
            "// Butler GPS Service Worker\n"
            "const CACHE = 'butler-gps-v1';\n"
...
class GPSServer:
    ...
    def relay_topic(self) -> str:
        """Unique ntfy.sh topic for this Butler instance."""
    ...
    async def start(self) -> bool:
        ...
        try:
            ...
            self._thread = threading.Thread(
                target=self._server.serve_forever, daemon=True, name="butler-gps",
            )
            self._thread.start()
            ip = _get_local_ip()
    ...
    def _start_relay_listener(self) -> None:
        ...
        self._relay_thread = threading.Thread(
            target=self._relay_listener_loop, daemon=True, name="butler-gps-relay",
        )
    ...
    def _relay_listener_loop(self) -> None:
        ...
        while self._relay_running:
            try:
                req = urllib.request.Request(
                    url, headers={"User-Agent": "Butler/2.0"}
                )
